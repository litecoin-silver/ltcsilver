...
double GetDifficultyLitecoin(const CBlockIndex* blockindex)
{
...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    if (blockindex->nHeight >= Params().GetConsensus().LTSHeight)
    {
        return GetDifficultyINTERNAL(blockindex);
    }
    else
    {
        return GetDifficultyLitecoin(blockindex);
    }
}
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    int ser_flags = (blockindex->nHeight < consensusParams.LTSHeight) ? SERIALIZE_BLOCK_LEGACY : 0;

    result.push_back(Pair("confirmations", confirmations));
    result.push_back(Pair("strippedsize", (int)::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS | ser_flags)));
