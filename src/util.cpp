...
const char * const BITCOIN_CONF_FILENAME = "ltcsilver.conf";
const char * const BITCOIN_PID_FILENAME = "ltcsilver.pid";
...
#else
    const char* pszModule = "ltcsilver";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ltcsilver";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ltcsilver";
#else
    // Unix
    return pathRet / ".ltcsilver";
#endif
#endif
}
