...
bool CanDirectFetch(const Consensus::Params &consensusParams)
{
    ...
    if (fLTSBootstrapping && consensusParams.LitecoinPostforkTime > 0) {
        target_time = consensusParams.LitecoinPostforkTime;
    }
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        LogPrintf("receive version message: %s: version %d, blocks=%d, us=%s, peer=%d%s\n",
                  ...
                  remoteAddr);
        if (pfrom->fUsesLTSMagic) {
            LogPrintf("peer %d uses LTS magic in its headers\n", pfrom->id);
        }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            if (fLTSBootstrapping) {
                stop_hash = chainparams.GetConsensus().LitecoinPostforkBlock;
            }
        ...
        if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast)) {
            if (fLTSBootstrapping && pindexLast != nullptr) {
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                uint256 stop_hash;
                if (fLTSBootstrapping) {
                    stop_hash = consensusParams.LitecoinPostforkBlock;
                }
