...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure)
{
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, Params().GetDefaultPort(fLTSBootstrapping), nConnectTimeout, &proxyConnectionFailed) :
                  ConnectSocket(addrConnect, hSocket, nConnectTimeout, &proxyConnectionFailed))
    {
        ...
        if (fLTSBootstrapping) {
            pnode->fUsesLTSMagic = false;
        }
    ...
    for (const CDNSSeedData &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, 0, true))
            {
                ...
                {
                    int nOneDay = 24*3600;
                    CAddress addr = CAddress(CService(ip, Params().GetDefaultPort(fLTSBootstrapping)), requiredServiceBits);
    ...
    while (!interruptNet)
    {
        ...
        while (!interruptNet)
        {
            ...
            if ((addr.GetPort() != Params().GetDefaultPort() ||
                (fLTSBootstrapping && addr.GetPort() != Params().GetDefaultPort(true))) && nTries < 50)
                continue;
    ...
    while (true)
    {
        ...
        for (const AddedNodeInfo& info : vInfo) {
            if (!info.fConnected) {
                ...
                tried = true;
                CService service(LookupNumeric(info.strAddedNode.c_str(), Params().GetDefaultPort(fLTSBootstrapping)));
